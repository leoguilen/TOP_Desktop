...
namespace Gerenciador.Forms
{
    public partial class AtualizaCadastro : Form
    {
        ...
        private void AtualizaCadastro_Load(object sender, EventArgs e)
        {
            ...
            foreach (var itemUser in from user in bd.v_relatorioUsuario.Where(u => u.id_user == IDSelecionado && u.desc_tipoCont.Contains("Telefone")) select user)
            {
                dadosUsuario = itemUser;
                tel = itemUser.desc_cont;
            }

            foreach (var itemUser in from user in bd.v_relatorioUsuario.Where(u => u.id_user == IDSelecionado && u.desc_tipoCont.Contains("Celular")) select user)
            {
                dadosUsuario = itemUser;
