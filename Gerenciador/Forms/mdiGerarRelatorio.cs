        ...
        public void GerarRelatorioPDF(TreeNodeCollection tree)
        {
            ...
            if (Directory.Exists(caminho))
            {
                foreach (TreeNode nome in tree)
                {
                    ...
                    var image = Image.GetInstance(@"C:\Users\LGuilen\Documents\Projetos C#\GerenciadorTOP\Gerenciador\Resources\Logo.png");
                    ...
                    foreach (TreeNode node in tree)
                    {
                                               ...
                                               select new { id.user_in_id })
                        {
                                                 ...
                                                 orderby inf.id_user ascending select inf)
                            {
                                ...
                                Phrase resExatas = new Phrase(string.Format("{0}%", info.exatas_resultado * 100));
                                Phrase resHumanas = new Phrase(string.Format("{0}%", info.humanas_resultado * 100));
                                Phrase resBio = new Phrase(string.Format("{0}%", info.biologicas_resultado * 100));
